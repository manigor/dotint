...
function drawDateCalendar($name,$value,$hidden=false,$tags='',$yearCase = false,$length=20,$hvalue=false,$extraEvent='',$minDate='',$maxDate=''){
	global $AppUI,$calendarsCount,$calCase,$totalCals;
	$AppUI->cals();
	$calCase = true;
	/*if($value != ''){
		//$hvalue=implode('',array_reverse(explode('/',$value)));				
	}*/
	if($hidden !== false){
		$visual='';
		$input2='<input type="text" class="hidden" name="'.$hidden.'" value="'.$hvalue.'">';
	}else {
		$visual='';
		$input2='';
	}
	
	if($tags != '' &&  preg_match('/id\s?=\s?"([^"]*)"/',$tags,$idsm)){
		$myID='';
		$hang=$idsm[1];
	}else {
		$myID='id="calField_'.$totalCals.'"';
		$hang='calField_'.$totalCals;
	}
	
	if($tags != '' &&  preg_match('/class\s?=\s?"([^"]*)"/',$tags,$extraClass)){
		$mclass=$extraClass[1];
	}else{
		$mclass='';
	}
	...
	$html='<span style="white-space:nowrap;">
				<input type="text" class="text spCals '.$mclass.'" name="'.$name. $visual . '" '.$tags.' value="'.$value.'" size="'.$length.'" readonly="readonly" '.$myID.'>
		  </span>';		
	$dateFormat = 'calObj';
	...
	$addlim=array();
	if($minDate !== ''){
		$addlim[]='t.minDate="'.$minDate.'";';
	}
	if($maxDate !== ''){
		$addlim[]='t.maxDate="'.$maxDate.'";';
	}
	if(!strstr($value,"new")){
		$value = '"'.$value.'"';
	}
	$AppUI->plainJs('var t=cloneThis('.$dateFormat.'); t.defaultDate='.$value.';'.join(" ",$addlim).'t.onClose=function(dates){'.$extraEvent.' };$j("#'.$hang.'").datepick( t );');
	$totalCals++;
	return $html;
}

function drawTimePicker($name,$value,$withDate,$extraClass = ''){
	global $AppUI;
	$code='<input type="text" class="text '.$extraClass.'" id="'.$name.'" name="'.$name.'" value="'.$value.'">';
	$defv='H:M';
	if($withDate === true){
		$defv.=' Y-O-D';
	}
	$AppUI->plainJs('$j("#'.$name.'").datetimeEntry({datetimeFormat: "'.$defv.'", defaultDatetime: "'.$value.'"});');
	return $code;
}
