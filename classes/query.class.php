...
class DBQuery {
   ...
   */
	...
	function loadHashListMine($index = null) {
		global $db;
	...
	function loadListExport($index = NULL,$round=false){
		global $db;
		if($round === false){
			if (! $this->exec(ADODB_FETCH_ASSOC)) {
				exit ($db->ErrorMsg());
		...
		$icnt=0;
		$tcnt=$this->round*500;
		while ($icnt < 500 && ($tcnt+$icnt) < $this->_query_id->_numOfRows) {
			$hash = $this->fetchRow();
			if($hash){
				if($first === true){
					$result=array_keys($hash);
					$first=false;
				}
				$hashlist[] = array_values($hash);
				++$icnt;
			}
		}
		if(($icnt < 500 && !$hash) || ($tcnt + $icnt) == $this->_query_id->_numOfRows  ){
			$this->clear();
			$do_return=false;
			$this->round=0;
		}elseif( ($tcnt + $icnt) < $this->_query_id->_numOfRows){
			$do_return=true;
		}
		return array($hashlist,$result,$do_return);
	}
	...
	function loadColumn($column_index=0) {
		global $db;
		if (! $this->exec(ADODB_FETCH_NUM)) {
			die ($db->ErrorMsg());
		}
	...
	function loadColumns($column_index=0) {
		global $db;
		if (! $this->exec(ADODB_FETCH_NUM)) {
			die ($db->ErrorMsg());
		}
