 ...
 */
	...
	*/
	 ...
	 */
	function loadJS() {
		global $m, $a, $dPconfig, $baseDir, $moduleScripts,$bottomJS,$calCase;
		...
		while ( list ( , $js_file_name ) = each ( $js_files ) ) {
			...
			$this->scriptJS ( "{$base}js/$js_file_name" );
		}
		$this->getModuleJS ( $m, $a, true );
		if (count ( $moduleScripts ) > 0) {
			foreach ( $moduleScripts as $jsFile ) {
				$this->scriptJS ( $jsFile );
			}
		}
			...
			echo '<script>'.
				...
				var existing = window.onload;
 				window.onload = function() {
					    ...
					    if(calObj){
						    var yearObj= cloneThis(calObj);
						    yearObj.dateFormat="yyyy";
						    yearObj.changeMonth = false;
						'.$bottomJS.'
						}
 				}
	...
	function loadCSS($module, $file = null, $load_all = false) {
		global $dPconfig, $baseDir, $module;
		if (! isset ( $module ) && $_REQUEST ['m'] != '') {
		...
		if (substr ( $base, - 1 ) != '/')
			$base .= '/';
		if ($load_all || ! $file) {
			if (file_exists ( "{$root}modules/$module/$module.module.css" ))
				echo "<link rel='stylesheet' type='text/css' href='{$base}modules/$module/$module.module.css'/>";
		}
	}
	...
	function getModuleJS($module, $file = null, $load_all = false) {
		...
		if ($load_all || ! $file) {
			if (file_exists ( "{$root}modules/$module/$module.module.js" ))
				...
				$this->scriptJS ( $base . "modules/$module/$module.module.js" );
		}
		...
		if (isset ( $file ) && file_exists ( "{$root}modules/$module/$file.js" ))
			...
			$this->scriptJS ( "{$base}modules/$module/$file.js" );
	}
	...
	function plainJs($jsString){
		global $bottomJS;
		$bottomJS.="\n".$jsString	."\n";
	}

	/*
	 * Function for control of amount of Calendars on the page
	 */
	function cals(){
		global $calendarsCount;
		return ++$calendarsCount;
	}
...
class CTabBox_core {
	...
	function add($file, $title, $translated = false, $key= NULL) {
		if (isset ( $key ))
			$this->tabs [$key] = array ($file, $title, $translated );
		else
			$lkey=array_push($this->tabs, array ($file, $title, $translated ));
		return (!is_null($key) ? $key : ($lkey-1));
...
class CTitleBlock_core {
	...
	/**
	 * Adds a table 'cell' to right-aligned bread-crumbs
	 *
	 * Cells are added from left to right.
	 */
	function addCrumbRight2($link, $label, $icon = '') {
		$this->crumbs [$link] = array ($label, $icon );
	}
