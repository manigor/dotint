(function (a) {
	a.widget("ui.dropdownchecklist", {version:function () {
		...
		e.css({display:"inline-block", "white-space":"nowrap", overflow:"hidden"});
		h.append(e);
		a("<div>", {class:"dropdown icns"}).appendTo(h);
		b.hover(function () {
			if (!j.disabled) {
		...
		var h = this, j = this.options, b = this.sourceSelect, d = this.dropWrapper;
		...
		if (a.isFunction(this.options.textFormatFunction)) {
			...
			if (d && (e != null) && e.attr("selected")) {
				...
				f = "";
				b.each(function () {
					if (a(this).attr("selected")  && !a(this).is(":disabled")) {
						if (f != "") {
							f += ", "
						}
		...
		var b = function (n) {
			if (!n.dropWrapper.isOpen) {
				...
				if ((g.positionHow == null) || (g.positionHow == "absolute")) {
					n.dropWrapper.css({position:"absolute", top:n.controlWrapper.position().top + n.controlWrapper.outerHeight() + "px", left:n.controlWrapper.position().left + "px"})
				} else {
					if (g.positionHow == "relative") {
						n.dropWrapper.css({position:"relative", top:"0px", left:"0px"})
					}
				}
				var m = 0;
				if (g.zIndex == null) {
		...
		if (d) {
			...
			e.addClass("inactive");
			b.addClass("ui-state-disabled").hide();
		} else {
			e.removeAttr("disabled");
			e.removeClass("inactive");
			e.addClass("active");
			b.removeClass("ui-state-disabled").show();
		}
		e.attr("checked", c)
	...
	a.extend(a.ui.dropdownchecklist, {defaults:{width:null, maxDropHeight:null, firstItemChecksAll:false, closeRadioOnClick:false, minWidth:50, positionHow:"absolute", bgiframe:false, explicitClose:null}})
})(jQuery);