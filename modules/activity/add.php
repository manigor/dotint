...
if ($activity_id > 0){
	...
	$q->addQuery("ac.activity_caregivers_caregiver_id as care_id,ac.activity_caregivers_other as other");
	$q->addWhere("ac.activity_caregivers_activity_id = \"". $activity_id.'" ');
	$activity_caregivers = $q->loadHashListMine();
	if(count($activity_caregivers ) > 0){
		$fhtml=array();
		foreach ($activity_caregivers as $key=> $vac) {
			if($vac['other'] == ''){
				$fhtml[]=$vac['care_id'];
			}else{
				$fhtml[]=$vac['other'];
		...
		$activity_caregivers_html=urlencode(implode(',',$fhtml));
	}
}
...
var selected_contacts_id = "<?php echo implode(',', $activity_contacts); ?>",
    ...
    selected_caregivers_id = "<?php echo $activity_caregivers_html; ?>",
    calendarField = '',
    calWin = null,
...
/*if(curamnt === 6){
  ...
  var row = $j("tr:eq(0)",$tbody);  // 1st row
	var newid = curamnt;  // Since this includes the header row, we don't need to add one
	var newrow = $j(row).clone(true);
  
  //rowrenumber(newrow, newid);
  $j(newrow)
    //.find(".nmb").text(5 + curamnt).end()
    .find("input").val("").end()
...
function DeleteRow(el){
  ...
  if (numrows == 2) {  // 2 rows before deleting - only 1 left now, so 'hide' delete button
	...
	/*if($j(ctable).attr("id") == 'facilitators'){
		var $tbody=$j(tbody);
		$j("tr",$tbody).each(function(i){
			$j(this).find("span.nmb").text(5 + i);
   ...
   function ajaxAssign(f) 
   {
     ...
     //var url = "?m=companies&a=do_ajaxowner_update&user=" + escape(user)+"&company_id="+escape(company);
		url = "modules/activity/do_ajaxclient_update.php?client=" + escape(client)+"&activity_id="+escape(activity);
	 //alert(url);	 
   ...
   <td align="left">
	<tr>
	  <td>
		...
		<tr>
			<td align="left">1d.<?php echo $AppUI->_('Hours per Day');?>:</td>
	 ...
	 <tr>
		<td align="left">2a.<?php echo $AppUI->_('Activity Name');?>:</td>
	 ...
	 <tr>
		<td align="left">3a.<?php echo $AppUI->_('No. of Persons (Male)');?>:</td>
     ...
     <tr>
		<td align="left">3b.<?php echo $AppUI->_('No. of Persons (Female)');?>:</td>
     ...
     <tr>
		<td align="left">3c.<?php echo $AppUI->_('Total');?>:</td>
     ...
     <tr>
		<td align="left">4.<?php echo $AppUI->_('Cadres Trained');?>:</td>
		<td align="left">
			<?php echo arraySelectCheckboxFlat($cadresType,'activity_cadres[]','',$obj->activity_cadres,$identifiers); 	?> 
		</td>
     </tr>
	  ...
	  <tr>
	  	<td align="left">11.<?php echo $AppUI->_('Notes');?>:</td>
		</tr>
		<tr>		
		<td colspan="2">
