...
$selected_client_ids = dPgetParam( $_REQUEST, 'selected_client_ids', 0 );
$selected_clients = explode(",", $$selected_client_ids );

//load client details and send back in xml string
$s = '';
foreach  ($selected_clients as $selected_client)
{
	$obj = new CClient();
	$obj->load($selected_client);
	$years = 0;
	$months = 0;
	$obj->getAge($years, $months);
	$s .= "<client><admissionno>$obj->client_adm_no</admissionno><name>".$obj->getFullName() . "</name><years>$years</years><months>$months</months></client>";
}
