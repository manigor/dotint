...
if ($canEdit) {
	$titleBlock->addCell ();
	$titleBlock->addCell ( '<input type="submit" class="button" value="' . $AppUI->_ ( 'new activity' ) . '" />', '', '<form action="?m=activity&a=add" method="post">', '</form>' );
}

$titleBlock->addCrumb ( "?m=activity", "Activities" );

if ($canEdit) {
	$titleBlock->addCrumb ( "?m=activity&a=add&activity_id=$activity_id", "Edit" );
...
//format date
$entry_date = intval ( $obj->activity_date ) ? new CDate ( $obj->activity_date ) : null; // intval($obj->activity_entry_date) ? new CDate($obj->activity_entry_date ) :  null;
$end_date = intval ( $obj->activity_end_date ) ? new CDate ( $obj->activity_end_date ) : null;
$titleBlock->show ();

$traineds = buildStringVals ( dPgetSysVal ( 'CadresTrained' ), $obj->activity_cadres );
...
<table border="0" cellpadding="4" cellspacing="0" width="65%"
	class="std">
...
<form name="frmDelete" action="./index.php?m=activity" method="post"><input
		type="hidden" name="dosql" value="do_newactivity_aed" /> <input
		type="hidden" name="del" value="1" /> <input type="hidden"
		name="activity_id" value="<?php echo $activity_id;?>" /></form>
<?php } ?>
<tr>
		<td valign="top"><strong><?php echo $AppUI->_('Details');?></strong>
		<table cellspacing="1" cellpadding="2">
			<tr>
				<td align="left" nowrap="nowrap">1a.<?php echo $AppUI->_('Center');?>:</td>
				<td class="hilite" width="50%"><?php echo $clinics[$obj->activity_clinic];?></td>
			</tr>			
			<tr>
				<td align="left" nowrap="nowrap">1b.<?php echo $AppUI->_('Start Date');?>:</td>
				<td class="hilite" width="50%"><?php if (isset ($entry_date)) echo $entry_date->format($df);?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">1c.<?php echo $AppUI->_('End Date');?>:</td>
				<td class="hilite" width="50%"><?php if (isset ($end_date)) echo $end_date->format($df);?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">1d.<?php echo $AppUI->_('Hours per Day');?>:</td>
				<td class="hilite"><?php echo $obj->activity_hpd;?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">2a.<?php echo $AppUI->_('Activity name');?>:</td>
				<td class="hilite" width="50%"><?php echo $obj->activity_description;?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">3a.<?php echo $AppUI->_('No. of persons (Male)');?>:</td>
				<td class="hilite" width="50%"><?php echo $obj->activity_male_count;?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">3b.<?php echo $AppUI->_('No. of persons (Female)');?>:</td>
				<td class="hilite"><?php echo $obj->activity_female_count;?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">3c.<?php echo $AppUI->_('Total');?>:</td>
				<td class="hilite"><?php echo $obj->activity_visiters_total;?></td>
			</tr>
			<tr>
				<td align="left" nowrap="nowrap">4.<?php echo $AppUI->_('Cadres trained');?>:</td>
				<td class="hilite"><?php echo $traineds;?></td>
			</tr>
			<tr>
		...
		<table>
		 ...
		 if (count($activities) > 0) {
			 foreach ($activities as $icount => $activityrecord) {
				
			 ?>
			 <tr>
						<td><?php echo (5 + $icount), 'a.&nbsp;' , $activityOptions[$activityrecord["facilitator_training_id"]];?></td>
						<td>b.&nbsp<?php echo $activityrecord["facilitator_topic"]?></td>
						<td>c.&nbsp<?php echo $activityrecord["facilitator_name"]?></td>						
					</tr>
			 <?php } 
...
$tabBox = new CTabBox( "?m=activity&a=view&activity_id=$activity_id", "", $tab );
