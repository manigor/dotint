...
function exportProceed(){
	...
	if ($clinic > 0) {
		...
		$wfile= new LTPExport('plain',$clinicName.'-LTP' );
		...
		foreach ( $list as $user => $dsa) {
			...
			//Last action
			$sql = 'update ltp_transfers set ondate=NOW() where client_id="' . $user . '" and clinic_id="'.$clinic.'" limit 1';
			$res = my_query ( $sql );
		}
		$wfile->close();
	}
