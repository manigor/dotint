...
function submitIt() {
	...
	if (form.clinic_name.value.length < 3) {
		alert( "<?php echo $AppUI->_('Invalid Center Name', UI_OUTPUT_JS);?>" );
...
/*
	if (isset($_GET['tab']))
		$AppUI->setState('ClinicAeTabIdx', dPgetParam($_GET, 'tab', 0));
	
	$moddir = $dPconfig['root_dir'] . '\\modules\clinics\\';	
	
	$tab = $AppUI->getState('ClinicAeTabIdx', 0);
	$tabBox =& new CTabBox("?m=clinics&a=addedit&clinic_id=$clinic_id", "", $tab, "");
	$tabBox->add($moddir. "ae_help", "Info");
	$tabBox->add($moddir. "ae_contacts", "Contacts");
	$tabBox->loadExtras('clinics', 'addedit');
	$tabBox->show('', true);
*/
?>
