...
$ttl = $company_id > 0 ? "Edit Center" : "New Center";
$titleBlock = new CTitleBlock( $ttl, '', $m, "$m.$a" );
$titleBlock->addCrumb( "?m=companies", "Centers" );
if ($company_id != 0)
  $titleBlock->addCrumb( "?m=companies&a=view&company_id=$company_id", "View" );
$titleBlock->show();
?>
...
<table>
	<tr>
		<td align="right"><?php echo $AppUI->_('Clinic Name');?>:</td>
		<td>
	...
	<tr>
		<td align="right"><?php echo $AppUI->_('Clinic Owner');?>:</td>
		<td>
	<?php
...
<?php
	...
	$tabBox =& new CTabBox("?m=companies&a=addedit_ks&company_id=$company_id", "", $tab, "");
	$tabBox->add($moddir. "ae_help", "Info");
	$tabBox->add($moddir. "ae_contacts", "Contacts");
	$tabBox->loadExtras('companies', 'addedit_ks');
	$tabBox->show('', true);
?>
