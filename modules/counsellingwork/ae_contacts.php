...
if ( count($contacts) > 0)
{
	...
	$q = new DBQuery;
	$q->addTable('company_contacts', 'cc');
	$q->leftJoin ('contact_types', 'ct', 'ct.typ_id = cc.company_contacts_contact_type');
	...
	$q->addWhere("cc.company_contacts_company_id = $company_id");
	$q->addWhere("cc.company_contacts_contact_id in ($contactsList)");
...
<form name="contactsEdit" action="?m=companies&a=addedit_ks&company_id=<?php echo $company_id; ?>" method="post">
<?php
if (count($contacts) == 0)
{
...
<?php
	...
	$w .= '&nbsp;<input type="button" class=button value="'.$AppUI->_( 'select pre-existing contact' ).'" onClick="javascript:window.location=\'./index.php?m=contacts&a=addedit&company_id='.$company_id.'&company_name='.$obj->company_name.'\'">';
	$w .= '</td></tr>';
	echo $w;
?>
</form>
