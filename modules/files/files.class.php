<?php /* FILES $Id: files.class.php,v 1.35.2.2 2006/02/10 19:47:52 gregorerhardt Exp $ */
...
class CFile extends CDpObject {
	...
	function delete() {
		...
		addHistory('files', $this->file_id, 'delete',  $this->file_name);
	// remove the file from the file system
		@unlink( "{$dPconfig['root_dir']}/files/$this->file_real_filename" );
	...
	function moveFile( $oldProj, $realname ) {
		...
		$res = rename("{$dPconfig['root_dir']}/files/$realname", "{$dPconfig['root_dir']}/files/$realname");
...
// move a file from a temporary (uploaded) location to the file system
	...
	// check that directories are created
		...
		$this->_filepath = "{$dPconfig['root_dir']}/files/$this->file_real_filename";
	// move it
	if(is_uploaded_file($upload['tmp_name'])){
		$res = move_uploaded_file( $upload['tmp_name'], $this->_filepath );	
	}else{
		$res = copy($upload['tmp_name'],$this->_filepath);
...
function shownavbar1($xpg_totalrecs, $xpg_pagesize, $xpg_total_pages, $page)
{
