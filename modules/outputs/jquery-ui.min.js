...
jQuery.tablesorter.addWidget({ 
    ...
    format: function(table) { 
        if(!this.headers) { 
            var h = this.headers = [];  
            $j("thead th",table).each(function(n) { 
        	if(n ==1 || n ==2 || n==3){
        	$j(this).width(function(i,x){
        	    return x+24;
        	});
        	}
            }); 
...
jQuery.tablesorter.addParser({id: "size", is: function(s) {return false; },
		    format: function(s) {
			var str = s.replace(/>.*$/,'>').match(/fsort="(.*)">/);
			if(str && str.length > 0){
		    	    return  str[1];
		    	}
		    },
...
jQuery.tablesorter.addParser({id: "idate", is: function(s) {return false; },
    format: function(s){
		var str = trim(s.replace(/<[a-zA-Z\/][^>]*>/g, '')).replace(/-/g,'');
		//alert(str+"||||"+s);
		return str;
	},
	type: "numeric"
});
jQuery.tablesorter.addParser({id: "idlink", is: function(s) {return false; },
    format: function(s){
		var str = trim(s.replace(/<[a-zA-Z\/][^>]*>/g, ''));
		return str;
	},
	type: "numeric"
});
jQuery.tablesorter.addParser({id: "itlink", is: function(s) {return false; },
    format: function(s){
		var str = trim(s.replace(/<[a-zA-Z\/][^>]*>/g, ''));
		return str;
	},
...
jQuery.tablesorter.addWidget({ 
    ...
    format: function(table) { 
        if(!this.headers) { 
            var h = this.headers = [];  
            $j(".header",table).each(function(n) {
    	    $j(this)
		        .width(function(i,x){
    	                return x+32;
    	            })
