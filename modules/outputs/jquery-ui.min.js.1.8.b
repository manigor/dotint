...
jQuery.ui || (function (b) {
	...
	b.ui = {version:"1.8rc2", plugin:{add:function (d, e, g) {
...
(function (a) {
	a.widget("ui.mouse", {options:{cancel:":input,option", distance:1, delay:0}, _mouseInit:function () {
		...
		var b = this.element.selectable("option", "noConflict") || this.element.sortable("option", "noConflict");
		if (b) {
			if (e.originalEvent.mouseHandled && !this.element.data("ignore-selectable")) {
				return
			}
		} else {
			if (e.originalEvent.mouseHandled) {
...
(function (a) {
	...
	a.extend(a.ui.draggable, {version:"1.8rc2"});
...
(function (a) {
	...
	a.extend(a.ui.droppable, {version:"1.8rc2"});
...
(function (c) {
	...
	c.extend(c.ui.resizable, {version:"1.8rc2"});
...
(function (a) {
	a.widget("ui.selectable", a.ui.mouse, {options:{appendTo:"body", autoRefresh:true, distance:0, filter:"*", tolerance:"touch"}, _create:function () {
		...
		var k = this;
		var j = k.element.selectable("option", "noConflict") || k.element.sortable("option", "noConflict") || false;
		if (j) {
			k.element.data("ignore-selectable", false);
			if (k.element.data("sortable") && !b.metaKey && a(b.originalTarget).closest(".ui-selectee").length) {
				k.element.sortable("enable");
				k.element.data("ignore-selectable", true)
			}
		}
		...
		var l = this.options;
		this.selectees = a(l.filter, this.element[0]);
		this._trigger("start", b);
		a(l.appendTo).append(this.helper);
		this.helper.css({"z-index":100, position:"absolute", left:b.clientX, top:b.clientY, width:0, height:0});
		if (l.autoRefresh) {
			this.refresh()
		}
		var e = this.element, f = this.selectees, d = a(b.originalTarget).closest(".ui-selectee"), i = e.data("selectable-starts-multiple"), c = f.index(d), g = (i) ? f.index(i) : null;
		e.data("selectable-starts-multiple", d);
		if (b.shiftKey && c >= 0 && g !== null) {
			var h = f.slice(((c > g) ? g : c), (g = (g < c) ? c : g) + 1);
			h.each(function (m) {
				var n = a(this);
				if (n.is(".ui-selected") && m !== 0 && m !== h.length - 1) {
					n.removeClass("ui-selected");
					n.addClass("ui-unselecting")
				} else {
					n.addClass("ui-selecting")
				}
			})
		}
		this.selectees.filter(".ui-selected").each(function () {
			var m = a.data(this, "selectable-item");
			m.startselected = true;
			if (!b.metaKey) {
				m.$element.removeClass("ui-selected");
				m.selected = false;
				m.$element.addClass("ui-unselecting");
				m.unselecting = true;
				k._trigger("unselecting", b, {unselecting:m.element})
			}
		});
		a(b.target).parents().andSelf().each(function () {
			var m = a.data(this, "selectable-item");
			if (m) {
				m.$element.removeClass("ui-unselecting").addClass("ui-selecting");
				m.unselecting = false;
				m.selecting = true;
				m.selected = true;
				k._trigger("selecting", b, {selecting:m.element});
				return false
			}
		})
	}, _mouseDrag:function (b) {
		var i = this;
		this.dragged = true;
		var h = i.element.selectable("option", "noConflict") || i.element.sortable("option", "noConflict") || false;
		if (h) {
			if (this.options.disabled || i.element.data("ignore-selectable")) {
				i.element.sortable("enable");
				return false
			}
		} else {
			if (this.options.disabled) {
		...
		var j = this.options;
		var d = this.opos[0], g = this.opos[1], c = b.pageX, f = b.pageY;
		if (d > c) {
			var e = c;
			c = d;
			d = e
		...
		this.helper.css({left:d, top:g, width:c - d, height:f - g});
		this.selectees.each(function () {
			...
			var l = false;
			if (j.tolerance == "touch") {
				l = (!(k.left > c || k.right < d || k.top > f || k.bottom < g))
			} else {
				if (j.tolerance == "fit") {
					l = (k.left > d && k.right < c && k.top > g && k.bottom < f)
				}
			}
			if (l) {
				if (k.selected) {
				...
				if (!k.selecting) {
					...
					k.selecting = true;
					i._trigger("selecting", b, {selecting:k.element})
				}
			} else {
				if (k.selecting) {
					if (b.metaKey && k.startselected) {
						...
						i._trigger("unselecting", b, {unselecting:k.element})
					}
				}
				if (k.selected) {
					if (!b.metaKey && !k.startselected) {
	...
	a.extend(a.ui.selectable, {version:"1.8rc2"})
})(jQuery);
(function (a) {
	a.widget("ui.sortable", a.ui.mouse, {widgetEventPrefix:"sort", options:{appendTo:"parent", axis:false, connectWith:false, containment:false, cursor:"auto", cursorAt:false, dropOnEmpty:true, forcePlaceholderSize:false, forceHelperSize:false, grid:false, handle:false, helper:"original", items:"> *", opacity:false, placeholder:false, revert:false, scroll:true, scrollSensitivity:20, scrollSpeed:20, scope:"default", tolerance:"intersect", zIndex:1000}, _create:function () {
		...
		var b = this.element.selectable("option", "noConflict") || this.element.sortable("option", "noConflict");
		if (b) {
			if (this.element.data("selectable") && f.metaKey) {
				return false
			}
		}
		if (this.reverting) {
	...
	a.extend(a.ui.sortable, {version:"1.8rc2"})
})(jQuery);
jQuery.effects || (function (g) {
	...
	g.extend(g.effects, {version:"1.8rc2", save:function (l, m) {
...
(function (a) {
	a.effects.fade = function (b) {
		return this.queue(function () {
			...
			c.animate({opacity:d}, {queue:false, duration:b.duration, easing:b.options.easing, complete:function () {
...
(function (a) {
	...
	a.extend(a.ui.accordion, {version:"1.8rc2", animations:{slide:function (j, h) {
...
(function (a) {
	a.widget("ui.autocomplete", {options:{minLength:1, delay:300}, _create:function () {
		...
		}}).zIndex(this.element.zIndex() + 1).css({top:0, left:0}).position({my:"left top", at:"left bottom", of:this.element, collision:"none"}).hide().data("menu");
		if (a.fn.bgiframe) {
			menu.element.bgiframe()
		}
	}, destroy:function () {
		this.element.removeClass("ui-autocomplete ui-widget ui-widget-content ui-corner-all").removeAttr("autocomplete").removeAttr("role").removeAttr("aria-autocomplete").removeAttr("aria-haspopup");
...
(function (a) {
	a.widget("ui.menu", {_create:function () {
		...
		var c = this;
		var b = this.element.children("li:not(.ui-menu-item)").addClass("ui-menu-item").attr("role", "menuitem");
...
(function (d) {
	...
	d.widget("ui.button", {options:{text:true, label:null, icons:{primary:null, secondary:null}}, _create:function () {
		...
		this.buttonElement.addClass(b).attr("role", "button").bind("mouseenter.button",
				function () {
					if (g.disabled) {
						return
					}
		...
		if (this.type === "checkbox") {
			this.buttonElement.bind("click.button", function () {
				...
				e.element.attr("checked", !e.element[0].checked).click();
			...
			if (this.type === "radio") {
				this.buttonElement.bind("click.button", function () {
					...
					d(this).addClass("ui-state-active");
					e.element.attr("checked", true).click();
	...
	d.widget("ui.buttonset", {_create:function () {
		...
		this.buttons = this.element.find(":button, :submit, :reset, :checkbox, :radio, a, .ui-button").button().map(
				function () {
					return d(this).button("widget")[0]
...
(function (b) {
	...
	b.widget("ui.dialog", {options:{autoOpen:true, buttons:{}, closeOnEscape:true, closeText:"close", dialogClass:"", draggable:true, hide:null, height:"auto", maxHeight:false, maxWidth:false, minHeight:150, minWidth:150, modal:false, position:"center", resizable:true, show:null, stack:true, title:"", width:300, zIndex:1000}, _create:function () {
		...
		this.element.css(d.height == "auto" ? {minHeight:Math.max(d.minHeight - c, 0), height:"auto"} : {height:Math.max(d.height - c, 0)});
		(this.uiDialog.is(":data(resizable)") && this.uiDialog.resizable("option", "minHeight", this._minHeight()))
	}});
	b.extend(b.ui.dialog, {version:"1.8rc2", uuid:0, maxZ:0, getTitleId:function (c) {
		return"ui-dialog-title-" + (c.attr("id") || ++this.uuid)
	}, overlay:function (c) {
		this.$el = b.ui.dialog.overlay.create(c)
	}});
	b.extend(b.ui.dialog.overlay, {instances:[], maxZ:0, events:b.map("focus,mousedown,mouseup,keydown,keypress,click".split(","),
			function (c) {
...
(function (f) {
	...
	f.fn.position = function (i) {
		...
		j.top += m[1];
		return this.each(function () {
			var u = f(this), s = u.outerWidth(), r = u.outerHeight(), p = f.extend({}, j), v, o, q;
			if (i.my[0] === "right") {
				p.left -= s
			} else {
				if (i.my[0] === e) {
			...
			f.each(["left", "top"], function (x, w) {
				if (f.ui.position[n[x]]) {
					f.ui.position[n[x]][w](p, {targetWidth:k, targetHeight:h, elemWidth:s, elemHeight:r, offset:m, my:i.my, at:i.at})
				}
			});
			if (f.fn.bgiframe) {
				u.bgiframe()
			}
			u.offset(f.extend(p, {using:i.using}))
		})
	};
...
(function (a) {
	...
	a.extend(a.ui.progressbar, {version:"1.8rc2"})
})(jQuery);
(function (b) {
	var a = 5;
	...
	b.extend(b.ui.slider, {version:"1.8rc2"})
})(jQuery);
(function (b) {
	var a = 0;
	...
	b.extend(b.ui.tabs, {version:"1.8rc2"});
...
(function (b) {
	...
	b.fn.tabs = function (d, e) {
		...
		return this.each(function () {
			var r = this;
			var p = b("ul." + e.navClass, r);
			p = p.size() && p || b(">ul:eq(0)", r);
			var B = b("a", p);
			if (e.remote) {
				B.each(function () {
					var C = e.hashPrefix + (++b.tabs.remoteCount), k = "#" + C, i = this.href;
					this.href = k;
					b('<div id="' + C + '" class="' + e.containerClass + '"></div>').appendTo(r);
					b(this).bind("loadRemoteTab", function (G, H) {
						var D = b(this).addClass(e.loadingClass), E = b("span", this)[0], F = E.innerHTML;
						if (e.spinner) {
							E.innerHTML = "<em>" + e.spinner + "</em>"
						}
						setTimeout(function () {
							b(k).load(i, function () {
								if (e.spinner) {
			...
			var u = b("div." + e.containerClass, r);
			u = u.size() && u || b(">" + e.tabStruct, r);
			p.is("." + e.navClass) || p.addClass(e.navClass);
			u.each(function () {
				var i = b(this);
				i.is("." + e.containerClass) || i.addClass(e.containerClass)
			});
			var n = b("li", p).index(b("li." + e.selectedClass, p)[0]);
			if (n >= 0) {
				e.initial = n
			}
			if (location.hash) {
				B.each(function (C) {
					if (this.hash == location.hash) {
						e.initial = C;
						if ((b.browser.msie || b.browser.opera) && !e.remote) {
							var k = b(location.hash);
							var D = k.attr("id");
							k.attr("id", "");
							setTimeout(function () {
								k.attr("id", D)
			...
			u.filter(":eq(" + e.initial + ")").show().end().not(":eq(" + e.initial + ")").addClass(e.hideClass);
			b("li", p).removeClass(e.selectedClass).eq(e.initial).addClass(e.selectedClass);
			B.eq(e.initial).trigger("loadRemoteTab").end();
			if (e.fxAutoHeight) {
				var A = function (i) {
					var k = b.map(u.get(),
							function (D) {
								...
								if (i) {
									if (b.browser.msie6) {
										D.style.removeExpression("behaviour");
										D.style.height = "";
										D.minHeight = null
									}
									C = E.css({"min-height":""}).height()
								} else {
									C = E.height()
								}
								return C
					...
					if (b.browser.msie6) {
						u.each(function () {
							this.minHeight = k[0] + "px";
							this.style.setExpression("behaviour", 'this.style.height = this.minHeight ? this.minHeight : "1px"')
						})
					} else {
						u.css({"min-height":k[0] + "px"})
					}
				};
				A();
				var q = r.offsetWidth;
				var z = r.offsetHeight;
				var j = b("#tabs-watch-font-size").get(0) || b('<span id="tabs-watch-font-size">M</span>').css({display:"block", position:"absolute", visibility:"hidden"}).appendTo(document.body).get(0);
				var s = j.offsetHeight;
			...
			var l = {}, v = {}, h = e.fxShowSpeed || e.fxSpeed, o = e.fxHideSpeed || e.fxSpeed;
			if (e.fxSlide || e.fxFade) {
				if (e.fxSlide) {
					l.height = "show";
					v.height = "hide"
				}
			...
			var m = e.onClick, g = e.onHide, w = e.onShow;
			B.bind("triggerTab", function () {
				var i = b(this).parents("li:eq(0)");
				if (r.locked || i.is("." + e.selectedClass) || i.is("." + e.disabledClass)) {
					return false
				}
				var C = this.hash;
				if (b.browser.msie) {
					...
					if (e.bookmarkable) {
						b.ajaxHistory.update(C);
						location.hash = C.replace("#", "")
					}
				} else {
					if (b.browser.safari) {
						var k = b('<form action="' + C + '"><div><input type="submit" value="h" /></div></form>').get(0);
						k.submit();
						b(this).trigger("click");
						if (e.bookmarkable) {
							b.ajaxHistory.update(C)
			...
			B.bind("disableTab", function () {
				var i = b(this).parents("li:eq(0)");
				if (b.browser.safari) {
					i.animate({opacity:0}, 1, function () {
						i.css({opacity:""})
					})
				}
				i.addClass(e.disabledClass)
			});
			if (e.disabled && e.disabled.length) {
				for (var y = 0, x = e.disabled.length; y < x; y++) {
					B.eq(--e.disabled[y]).trigger("disableTab").end()
				}
			}
			B.bind("enableTab", function () {
				var i = b(this).parents("li:eq(0)");
				i.removeClass(e.disabledClass);
				if (b.browser.safari) {
					i.animate({opacity:1}, 1, function () {
						i.css({opacity:""})
					})
				}
			});
			B.bind("click", function (E) {
				var J = E.clientX;
				var F = this, K = b(this).parents("li:eq(0)"), k = b(this.hash), G = u.filter(":visible");
				if (r.locked || K.is("." + e.selectedClass) || K.is("." + e.disabledClass) || typeof m == "function" && m(this, k[0], G[0]) === false) {
					this.blur();
					return false
				}
				r.locked = true;
				if (k.size()) {
					if (b.browser.msie && e.bookmarkable) {
						var i = this.hash.replace("#", "");
						k.attr("id", "");
						setTimeout(function () {
							k.attr("id", i)
						}, 0)
					}
					var D = {display:"", overflow:"", height:""};
					if (!b.browser.msie) {
						D.opacity = ""
					}
					function C() {
						if (e.bookmarkable && J) {
							b.ajaxHistory.update(F.hash)
						}
						G.animate(v, o, function () {
							b(F).parents("li:eq(0)").addClass(e.selectedClass).siblings().removeClass(e.selectedClass);
							G.addClass(e.hideClass).css(D);
							if (typeof g == "function") {
								g(F, k[0], G[0])
							}
							if (!(e.fxSlide || e.fxFade || e.fxShow)) {
								k.css("display", "block")
							}
							k.animate(l, h, function () {
								k.removeClass(e.hideClass).css(D);
								if (b.browser.msie) {
									G[0].style.filter = "";
									k[0].style.filter = ""
								}
					...
					if (!e.remote) {
						...
						b(F).trigger("loadRemoteTab", [C])
					}
				} else {
					alert("There is no such container.")
				}
				var I = window.pageXOffset || document.documentElement && document.documentElement.scrollLeft || document.body.scrollLeft || 0;
				var H = window.pageYOffset || document.documentElement && document.documentElement.scrollTop || document.body.scrollTop || 0;
				setTimeout(function () {
					window.scrollTo(I, H)
				}, 0);
				this.blur();
				return e.bookmarkable && !!J
			});
			if (e.bookmarkable) {
				b.ajaxHistory.initialize(function () {
					B.eq(e.initial).trigger("click").end()
				})
			}
		})
	};
	...
	for (var a = 0; a < c.length; a++) {
		b.fn[c[a]] = (function (d) {
			return function (e) {
				return this.each(function () {
					var g = b("ul.tabs-nav", this);
					g = g.size() && g || b(">ul:eq(0)", this);
					var f;
					if (!e || typeof e == "number") {
						f = b("li a", g).eq((e && e > 0 && e - 1 || 0))
					} else {
						if (typeof e == "string") {
							f = b('li a[@href$="#' + e + '"]', g)
						}
	...
	b.fn.activeTab = function () {
		var d = [];
		this.each(function () {
			var f = b("ul.tabs-nav", this);
			f = f.size() && f || b(">ul:eq(0)", this);
			var e = b("li", f);
			d.push(e.index(e.filter(".tabs-selected")[0]) + 1)
		});
		return d[0]
	}
})(jQuery);
...
jQuery.tablesorter.addParser({id:"size", is:function (s) {
	return false;
},
	format:function (s) {
...
jQuery.tablesorter.addParser({id:"idate", is:function (s) {
	return false;
},
	format:function (s) {
...
jQuery.tablesorter.addParser({id:"idlink", is:function (s) {
	return false;
},
	format:function (s) {
...
jQuery.tablesorter.addParser({id:"itlink", is:function (s) {
	return false;
},
	format:function (s) {
		var str = trim(s.replace(/<[a-zA-Z\/][^>]*>/g, ''));
