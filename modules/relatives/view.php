...
$titleBlock = new CTitleBlock( 'View Organisation', '', $m, "$m.$a" );
if ($canEdit) {
	$titleBlock->addCell();
	$titleBlock->addCell(
		'<input type="submit" class="button" value="'.$AppUI->_('new organisation').'" />', '',
...
$titleBlock->addCrumb( "?m=companies", "organisations list" );
if ($canEdit) {
	$titleBlock->addCrumb( "?m=companies&a=addedit&company_id=$company_id", "edit this organisation" );
	
	if ($canDelete) {
		$titleBlock->addCrumbDelete( 'delete organisation', $canDelete, $msg );
	}
}
$titleBlock->show();
?>
...
?>
function delIt() {
	if (confirm( "<?php echo $AppUI->_('doDelete').' '.$AppUI->_('Organisation').'?';?>" )) {
