<?php  // $Id: systemconfig.php,v 1.7 2005/03/15 07:25:59 ajdonnison Exp $
...
foreach ($rs as $c) {
	...
	$value = '';
	if(!strstr($c['config_name'],'regular_')){
		...
		switch ($c['config_type']) {
			case 'select':
				...
				if($c['config_name']!='current_center'){
					...
					foreach ($clinics as $cid =>  $citem) {
						if($c['config_value']  == $citem){
							$selv='selected="selected"';
						}else{
							$selv='';
						}
						$entry.='<option '.$selv.'  value="'.$citem.'">'.$citem.'</option>';
					}
				}
				$entry .= "</select>";
	...
	$output .= "<tr>
			<td class='item' width='20%'>".$AppUI->_($c['config_name'].'_title')."</td>
            		<td align='left'>
				$entry
				<a href='#' onClick=\"ldp('{$c['config_name']}')\" $tooltip>(?)</a>
...
<script>
function ldp(it){
	window.open('?m=system&a=systemconfig_help&dialog=1&cn='+it, 'contexthelp', 'width=400, height=200, left=50, top=50, scrollbars=yes, resizable=yes');
}
</script>