<?php /* TASKS $Id: organizer.php,v 1.17 2003/12/11 20:30:21 gregorerhardt Exp $ */
		...
		if($task2["fixed"] && (
			...
			if(!$option_advance_if_possible || $task2["task_percent_complete"] != 100) {
				...
				if ( $option_check_vital_users ) {
					...
					$vital = my_num_rows(my_query($sql1)) == 2 && my_num_rows(my_query($sql2)) > 0;
				} else {
					$vital = true;
				}
...
function get_last_children($task) {
	...
	$query = my_query($sql);
	if(my_num_rows($query)) {
		// has children
		while($row = my_fetch_array($query)) {
...
function process_dependencies($i) {
	...
	$query = my_query("select tasks.* from tasks,task_dependencies where task_id=dependencies_req_task_id and dependencies_task_id=" . $tasks[$i]["task_id"]);

	if(my_num_rows($query) != 0) {
		$all_fixed = true;
...
if($do != "conf") {
	...
	while ($row = my_fetch_array( $dtrc, my_ASSOC )) {
	...
	if($option_fix_task_group_date_ranges) {
		...
		$taskgroups = my_query($sql);
		while($tg = my_fetch_array($taskgroups)) {
			...
			if (db_dateTime2unix($tg["task_start_date"]) != $min_time
				...
				if ($do == "ask") {
					...
					my_query("update tasks set task_start_date='" .  db_unix2dateTime($min_time) . "', task_end_date='" .  db_unix2dateTime($max_time) . "' where task_id=" . $tg["task_id"]);
				}
			}
		}
	}
