...
if (isset($training_id) && $training_id > 0)
{
	$q = new DBQuery;
	...
	$q->innerJoin('training_clients', 'ac', 'ac.training_clients_client_id = c.client_id');
	$q->addQuery('c.client_id, c.client_first_name, c.client_status, c.client_notes, c.client_phone1');
	...
	if ((count($allowedClinics) > 0) && ($allowedClinics[0] <> NULL)) { $q->addWhere('c.client_clinic IN (' . implode(',', array_keys($allowedClinics)) . ')'); }
	...
	$q->addWhere('ac.training_clients_training_id = ' . $training_id);
...
<tr>
	<th nowrap="nowrap">
		<a href="?m=activity&orderby=client_adm_no" class="hdr"><?php echo $AppUI->_('Adm. No');?></a>
	</th>	
	<th nowrap="nowrap">
		<a href="?m=activity&orderby=client_last_name" class="hdr"><?php echo $AppUI->_('Client Name');?></a>
	</th>
	<th nowrap="nowrap">
		<?php echo $AppUI->_('Age (years)');?>
	</th>		
	<th nowrap="nowrap">
		<?php echo $AppUI->_('Age (months)');?>
	</th>	
...
if ( $AppUI->isActiveModule('clients') && $perms->checkModule('clients', 'view')) {
							$s .= "<input type='button' class='button' value='".$AppUI->_("Select clients...")."' onclick='javascript:popClients();' />";
						}
