...
$types = dPgetSysVal('ContactStatus');
$search = false;
...
if (strncmp($currentTabName,"All Contacts", strlen("All Contacts")) == 0)
	$clientType = false;
if (strncmp($currentTabName,"Not Applicable", strlen("Not Applicable")) == 0)
...
if (isset($activity_id) && $activity_id > 0)
{
	...
	//$q->innerJoin('activity_clients', 'acl', 'acl.activity_clients_activity_id = ac.activity_contacts_activity_id');
	//$q->innerJoin('clients', 'cl', 'cl.client_id = acl.activity_clients_client_id');
	...
	//$q->addWhere('acl.activity_clients_activity_id = ' . $activity_id);
	$q->addOrder($contactorderby.' '.$contactorderdir);
...
<tr>
	<th nowrap="nowrap">
		<?php echo $AppUI->_('Contact ID');?>
	</th>	
	<th nowrap="nowrap">
		<?php echo $AppUI->_('Staff Name');?>
	</th>	
...
{	foreach ($rows as $row)
	{
		...
		$none = false;
		$s .= $CR . '<tr>';
		$s .= $CR . '<td nowrap="nowrap"><a href="index.php?m=contacts&a=view&contact_id='. $row["contact_id"] .'">' . $row["contact_id"] .'</a></td>';
...
if ($none)
{
	echo $CR . '<tr><td colspan="8">' . $AppUI->_( 'No staff available' ) . '</td></tr>';
}
?>
</table>
<?php
